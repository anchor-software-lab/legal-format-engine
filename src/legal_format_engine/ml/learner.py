...
The approach is deliberately NOT neural-network based. Legal formatting is
deterministic — courts have rules. The "learning" is about discovering what
those rules actually look like in practice, by observing many real documents.
...
Outlier rejection is critical: one brief with 8pt font or 3" margins shouldn't
...
from legal_format_engine.ml.features import DocumentFeatures, FeatureValue
...
@dataclass
class LearnedFormat:
    """Complete learned formatting specification for a jurisdiction/court/doctype.

    This is the output of the learner. It can be used to:
    1. Generate a suggested YAML ruleset
    2. Override defaults in the formatting engine
    3. Validate that a brief matches the expected format
    """
    ...
    # Headings
    heading_styles: list[LearnedHeadingStyle] = field(default_factory=list)

    # Sections
    section_order: list[LearnedSectionOrder] = field(default_factory=list)
...
class FormatLearner:
    ...
    def learn(self) -> LearnedFormat:
        ...
        result.jurisdiction = self._features[0].jurisdiction
        result.court_level = self._features[0].court_level
