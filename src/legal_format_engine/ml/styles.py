"""Style profiles: learned formatting preferences per author or firm.

Attorneys and firms develop distinct formatting styles within the bounds
of court rules. This module tracks those preferences so the engine can:

1. Apply an author's/firm's preferred style when formatting new documents
2. Detect when a formatting choice is a deliberate style vs. a mistake
3. Offer "format like [firm]" as a feature

Style profiles are built from ingested documents tagged with author/firm.
They capture ONLY discretionary choices — things the court rules don't
specify. If a rule mandates 1" margins, the profile doesn't store margins.
...
- Whether to use Oxford comma in certifications
- Heading numbering style when rule doesn't specify
- Caption formatting details
...
class StylePreference:
    ...
    sample_count: int  # how many documents established this preference
    consistency: float  # 0-1, how consistently the author uses this value


@dataclass
class StyleProfile:
    """A collection of formatting preferences for an author or firm.

    Profiles only contain discretionary choices — formatting decisions
    where the court rules are silent.
    """
    profile_type: str  # "author" or "firm"
    name: str  # author name or firm name
...
# These are safe to learn from individual authors/firms.
...
class StyleProfileStore:
    ...
    def __init__(self, store_dir: Path | None = None):
        self._dir = store_dir or Path.home() / ".legal-format-engine" / "style_profiles"
...
def build_style_profile(
    ...
    """Build a style profile from a set of documents by the same author/firm.

    Args:
        documents: List of (NormalizedDocument, storage_record_dict) tuples
        profile_type: "author" or "firm"
        name: Author or firm name
        rule_governed_fields: Set of field names that are governed by court
    ...
    profile.jurisdictions = sorted(jurisdictions)
