"""Google Docs connector for importing documents into the legal format engine.
...
class GoogleDocsConnector:
    ...
    def download_as_docx(share_url: str, output_path: Path | None = None) -> Path:
        ...
        Converts a Google Docs share URL to an export URL::
        ...
        Raises:
            ...
            ConnectionError: If the download fails (not shared publicly,
                network error, etc.).
        """
        ...
        try:
            req = urllib.request.Request(
                export_url,
                headers={"User-Agent": "LegalFormatEngine/1.0"},
            )
    ...
    def parse_html_export(html_path: Path) -> dict:
        ...
        Google Docs HTML exports have a predictable structure:

        - A ``<style>`` tag containing CSS class definitions
          (``.c0``, ``.c1``, …) that encode fonts, sizes, weights, etc.
        - Body content that references those classes on ``<p>`` and
          ``<span>`` tags.
        - Optional ``@page`` rules with margin information.

        Returns:
            A dict suitable for feeding into ``BriefAnalysis`` /
            ``ProcessedDocument``, with keys:
...
class _GoogleDocsHTMLParser(HTMLParser):
    ...
    def __init__(self) -> None:
        ...
        # CSS class -> property dict  e.g. {"c0": {"font-family": "Arial", ...}}
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if self._in_body and tag == "span":
            # Flush any pending text into a span
            self._flush_span()
    ...
    def handle_endtag(self, tag: str) -> None:
        ...
        if self._in_body and tag == "p" and self._current_para is not None:
            ...
            self._current_para["text"] = "".join(
                s["text"] for s in self._current_spans
            )
    ...
    def _parse_css(self, css_text: str) -> None:
        """Extract class definitions and @page rules from CSS text."""
        # Class definitions
    ...
    # -- Resolve styles for a set of CSS classes -----------------------

    def _resolve_styles(self, classes: list[str]) -> dict[str, str]:
        """Merge CSS properties from a list of class names."""
        merged: dict[str, str] = {}
        for cls in classes:
    ...
    def result(self) -> dict:
        """Build the final extraction result after parsing is complete."""
    ...
    def _build_paragraphs(self) -> list[dict]:
        """Convert raw paragraph data into enriched dicts with resolved
        styles."""
        results: list[dict] = []
        for raw in self._paragraphs:
            ...
            # Resolve paragraph-level styles
            para_styles = self._resolve_styles(raw["classes"])

            # Resolve span-level styles to detect bold / italic
            ...
            # Determine dominant font for the paragraph
    ...
    def _build_font_summary(self, paragraphs: list[dict]) -> dict:
        """Compute dominant font name and size across paragraphs."""
    ...
    def _detect_headings(self, paragraphs: list[dict]) -> list[dict]:
        ...
        for idx, para in enumerate(paragraphs):
            ...
            is_all_caps = (
                bool(alpha_chars) and all(c.isupper() for c in alpha_chars)
            )
    ...
    def _build_page_settings(self) -> dict:
        """Convert @page CSS properties to a structured dict."""
        result: dict[str, object] = {}

        # Parse margin shorthand or individual margins
        ...
        for side in ("top", "right", "bottom", "left"):
            ...
            if key in self._page_settings:
                result[f"margin_{side}"] = _css_size_to_inches(
                    self._page_settings[key]
                )
...
def _css_size_to_pt(value: str) -> float:
    """Convert a CSS size value to points. Returns 0.0 on failure."""
    if not value:
        return 0.0
    ...
    if unit == "pt":
        return num
    if unit == "px":
        return num * 0.75  # 1px ≈ 0.75pt at 96dpi
    if unit == "in":
        return num * 72
    if unit == "cm":
        return num * 28.3465
...
def _dominant_span_value(spans: list[dict], key: str) -> object | None:
    """Return the most common non-empty value for *key* across spans,
