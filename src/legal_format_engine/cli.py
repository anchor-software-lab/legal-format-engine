...
def format(
    ...
    Reads INPUT_FILE (plain text), applies formatting rules, and
    writes the formatted output to --output.
    """
    # Load metadata
    ...
    # Load ruleset
    try:
        ruleset = load_ruleset(jurisdiction, court_level, doc_type)
...
def caption(metadata: str, jurisdiction: str, court_level: str, doc_type: str) -> None:
...
@click.option("--input-file", "-i", type=click.Path(exists=True), required=True,
              help="Plain text file to validate.")
@click.option("--jurisdiction", default="wisconsin")
@click.option("--court-level", default="appellate")
@click.option("--doc-type", default="brief")
def validate(
    metadata: str, input_file: str, jurisdiction: str, court_level: str, doc_type: str
) -> None:
