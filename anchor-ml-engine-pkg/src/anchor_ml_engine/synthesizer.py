...
2. A diff against the existing ruleset showing what the ML would change
3. User-facing recommendations with confidence levels

The key principle: the synthesizer NEVER silently overrides mandatory rules.
It produces suggestions that a human (or the engine with user approval) can
apply. Mandatory rules are the floor -- ML can refine within them but not violate.
"""
...
def synthesize_ruleset(
    ...
    Returns a dict matching a ruleset schema structure that can be
    written to YAML or loaded directly.
    """
    category = category or learned.category or "learned"
    subcategory = subcategory or learned.subcategory or "default"
    document_type = document_type or learned.document_type or "document"
